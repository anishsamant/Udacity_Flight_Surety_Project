...
function getOracleAccounts() {
	return new Promise((resolve, reject) => {
		...
		// We start at account 20 so we have first ones usable for airline and passengers.
		oracleAccounts = accountList.slice(0, TEST_ORACLES_COUNT);
		}).catch(err => {
			reject(err);
		}).then(() => {
